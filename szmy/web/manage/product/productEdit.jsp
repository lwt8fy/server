...
<%@page import="net.jeeshop.services.manage.company.bean.Company"%>
...
if(comType!=null&&comType.equals("2")){
	request.getRequestDispatcher("/manage/product/productEdit2.jsp").forward(request,response);
}
Company company=(Company)request.getSession().getAttribute("manage_session_company_info");
if(company!=null&&company.getType()!=null&&company.getType().equals("2")){
	request.getRequestDispatcher("/manage/product/productEdit2.jsp").forward(request,response);
}
...
<s:form action="product" id="form" name="form" namespace="/manage" theme="simple" enctype="multipart/form-data" method="post">
	...
	<input type="hidden" name="e.chuChangPrice" id="chuChangPrice" />
...
<!-- 			<a href="product!selectList.action?init=y" class="btn btn-inverse">返回</a> -->
			...
			<div id="tabs-1">
				<table class="table table-condensed">
							<tr style="display: none;">
								<td>id</td>
								<td>
								<s:hidden name="e.id" label="id" id="id"/>
		...
		<!-- 					</tr> -->
							...
							<tr id="clDiv">
								...
								<td style="text-align: left;"><s:textfield name="e.price"  onkeyup="constraintNumber(this);" data-rule="定价;required;price;" size="10" maxlength="10"
										id="price" />元</td>
								<td style="text-align: right;">现价</td>
								<td style="text-align: left;"><s:textfield name="e.nowPrice"  onkeyup="constraintNumber(this);" data-rule="现价;required;nowPrice;"   size="10" maxlength="10" 
										id="nowPrice" />元</td>
							</tr>
							...
							<tr>
								...
								<s:textfield name="e.stock" data-rule="库存;required;integer;stock;" id="stock" /></td>
								
							</tr>
							</s:else>
							<tr>
								<td style="text-align: right;">物流模版</td>
								<td style="text-align: left;" colspan="3">
									<input type="hidden" id="logid" value="${e.logisticsid}">
									<select name="e.logisticsid" id="logisticsid" >
										<option>--选择物流--</option>
									</select>
								</td>
							</tr>
					...
					<s:if test="e.specList!=null and 1==1">
						<s:iterator value="e.specList" var="item" status="stat">
							<tr>
								...
								<s:hidden name="e.specList[%{#stat.index}].purchasePrice"  id="sp%{#stat.index}" cssClass="search-query input-small"/>
								<s:textfield name="e.specList[%{#stat.index}].specPrice" onkeyup="changeSpecPrice2(%{#stat.index},$(this).val());"  cssClass="search-query input-small"/>
								</th>
								<s:if test="e.activityID!=null and !e.activityID.equals(\"\")">
...
//'fileTypeExts' : '*.jpg;*.bmp;*.png;*.gif', //控制可上传文件的扩展名，启用本项时需同时声明filedesc
	...
	function checkPrice(){
		...
		if($("#nowPrice").val()<=0){
			...
			$('#chuChangPrice').val($('#nowPrice').val());
		return true;
		}
		}else{
			$('#chuChangPrice').val($('#nowPrice').val());
	...
	*/
	function changeSpecPrice2(id,purchasePrice){
		...
		document.getElementById(id).value=chuChangprice;
	}
	
	function selectLogisticsList(){
		var logid=$('#logid').val();
		var _url = "../manage/logistics!selectListForSelect.action";
		$.ajax({
	        dataType:'json',
	        type: 'POST',
	        url : _url,
	        ...
	        success : function(data){
	        	if(data.length>0){
	        		for ( var i = 0; i < data.length; i++) {
	        			if(logid==data[i].id){
		        			$('#logisticsid').append('<option value="'+data[i].id+'" selected="selected">'+data[i].name+'</option>');
		        		}else{
		        			$('#logisticsid').append('<option value="'+data[i].id+'">'+data[i].name+'</option>');
		        		}
					}
			 	}
			}
