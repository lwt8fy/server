...
<!-- 							> -->
			...
			<tr>
				...
				<td style="text-align: left;" colspan="7">
						...
						<s:select list="#{0:'',1:'未审核',2:'已通过',3:'未通过'}" id="checkState" name="e.checkState"  cssClass="input-small" 
						listKey="key" listValue="value"  />
...
<%-- 						<s:submit method="updateDown" value="下架" cssClass="btn btn-warning" onclick="return downSelect();"/> --%>
					...
					<%} %>
					<s:if test="#session.manage_session_user_info.rid==1"> 
						<button method="product!updatePassAll.action" class="btn btn-success" onclick="return submitIDs(this,'确定通过选择的记录?');">
							批量通过
						</button>
					</s:if>
...
<!-- 							<font color='blue'>特价</font> -->
					...
					<td>
						<s:if test="checkState==1 or checkState==null or checkState==0">
						未审核
						</s:if>
						<s:elseif test="checkState==2">
						 已通过
						</s:elseif>
						<s:elseif test="checkState==3">
						<a href="javascript:alert('<s:property value="reason" />')">
						未通过(查看)
						</a>
						</s:elseif>
					</td>
					...
					<a href="product!toEdit.action?e.id=<s:property value="id"/>"  onclick="selectColor(this);"  target="_blank">编辑 </a>
					 |
					<a target="_blank" href="<%=request.getContextPath() %>/product/<s:property value="id"/>.html">查看</a>
...
<link rel="stylesheet" href="<%=request.getContextPath()%>/js/ymPrompt/skin/vista/ymPrompt.css"  type="text/css">
<SCRIPT type="text/javascript" src="<%=request.getContextPath()%>/js/ymPrompt/ymPrompt.js"></SCRIPT>

<c:if test="${message!=null && message!=''}"> 
    <script language="javascript">
    ymPrompt.confirmInfo({message:'${message}',handler:function(b){
        window.location.href="<%=request.getContextPath()%>/manage/product!selectList.action";
    	  }})
    </script>
</c:if>