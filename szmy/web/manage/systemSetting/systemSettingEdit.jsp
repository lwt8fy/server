...
<%-- 	</s:a> --%>
	...
	<s:if test="#session.isStartSocket==1">  
	<s:a   cssClass="btn" disabled="disabled">
		已启动生猪交易聊天服务
	</s:a>
	</s:if>
	<s:else>
		<s:a   cssClass="btn btn-warning" onclick="start();">
		启动生猪交易聊天服务
		</s:a>
	</s:else>
	 -->
...
<input value="<%=request.getContextPath() %>/index.jsp?style=" id="base_input" style="display: none;"/>
...
function start(){
	$.post("<%=request.getContextPath()%>/manage/systemSetting!startSocket.action",
            { fileName: '' }, 
            function (data) {});
    window.location.href="<%=request.getContextPath()%>/manage/systemSetting!toEdit.action";
}
</script>
