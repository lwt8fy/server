...
public class CountUtil {
	...
	public synchronized void addcount(Date da) {// 比较日期增加计数
		...
		if (new SimpleDateFormat("yyyy-MM-dd").format(CountUtil.obj.date).equals(
				new SimpleDateFormat("yyyy-MM-dd").format(da)))
			CountUtil.obj.setDayCount(CountUtil.obj.getDayCount() + 1);
		else
			CountUtil.obj.setDayCount(1);

		if (new SimpleDateFormat("yyyy-MM").format(CountUtil.obj.date).equals(
				new SimpleDateFormat("yyyy-MM").format(da)))
			CountUtil.obj.setMonthCount(CountUtil.obj.getMonthCount() + 1);
		else
			obj.setMonthCount(1);
		...
		if (ca.get(Calendar.DAY_OF_WEEK) == Calendar.MONDAY
				&& !new SimpleDateFormat("yyyy-MM-dd").format(CountUtil.obj.date)
						.equals(new SimpleDateFormat("yyyy-MM-dd").format(da)))
			obj.setWeekCount(1);
		else
			obj.setWeekCount(obj.getWeekCount() + 1);

		if (new SimpleDateFormat("yyyy").format(CountUtil.obj.date).equals(
				new SimpleDateFormat("yyyy").format(da)))
			CountUtil.obj.setYearCount(CountUtil.obj.getYearCount() + 1);
		else
			obj.setYearCount(1);
		obj.setDate(da);
