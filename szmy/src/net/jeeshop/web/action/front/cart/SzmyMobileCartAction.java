 ...
 * 
 */
public class SzmyMobileCartAction extends BaseAction<CartInfo> {
...
//				cartInfo.setAmount(cartInfo.totalCacl());
		...
		Map<String, Object>  compList = (HashMap<String, Object>)this.getSession().getAttribute("compList");
		if(compList==null){
			compList = new HashMap<String, Object>();
		}
		if(dele!=null){
			int i =0;
			for(Iterator<Product> it = cartInfo.getProductList().iterator();it.hasNext();){
				Product p = it.next();
				if(p.getCompID().equals(dele.getCompID())){
					i=1;
					break;
				}
			}
			if(i==0){
				compList.remove(dele.getCompID());
			}
		}
		return "cart";
	}
...
//		}
		...
		String buySpecID = getRequest().getParameter("specId");//选中的规格ID
...
//		product.setStatus(1);
		...
		compList.put(product.getCompID(), product.getCompName());
		this.getSession().setAttribute("compList", compList);
		...
		for(int i=0;i<cartInfo.getProductList().size();i++){
			Product p = cartInfo.getProductList().get(i);
			if(p.getId().equals(productID)){
				if((p.getBuySpecInfo()==null&&product.getBuySpecInfo()==null)||(p.getBuySpecInfo()!=null&&product.getBuySpecInfo()!=null&&p.getBuySpecInfo().getId().equals(product.getBuySpecInfo().getId()))){
...
//				}
				...
				//判断如果是积分商品，则计算所需的积分
				if(activity.getActivityType().equals(net.jeeshop.services.common.Activity.activity_activityType_j)){
...
//			super.write("{\"code\":\"notThisProduct\",\"msg\":\"商品已卖完或已下架，请联系站点管理员或从购物车中删除!\"}");
			...
			if(activity==null || activity.getStatus().equals(net.jeeshop.services.common.Activity.activity_status_n)){
...
//			throw new NullPointerException("请先登陆！");
		...
		if(type!=null&&type.equals("buyNow")){
			cartInfo = (CartInfo)getSession().getAttribute(FrontContainer.myBuyNow);
		}else{
			cartInfo = (CartInfo)getSession().getAttribute(FrontContainer.myCart);
		}
		if(cartInfo==null){
			logger.error("login..");
		...
		result.code = "result";
		List<StockErrorProduct> list = new LinkedList<SzmyMobileCartAction.StockErrorProduct>();
		for(int i=0;i<cartInfo.getProductList().size();i++){
			Product pro = cartInfo.getProductList().get(i);
